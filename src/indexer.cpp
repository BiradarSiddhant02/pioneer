...
Indexer::Indexer(const IndexerConfig& config) : config_(config) {
    ...
    if (config_.num_threads == 0) {
        ...
        if (config_.num_threads == 0) config_.num_threads = 4;  // Fallback
    }
}
...
bool Indexer::parse_file(const fs::path& filepath,
    ...
    if (!parser || !parser->parse(source)) return false;
...
void Indexer::worker_parse_files(const std::vector<fs::path>& files,
